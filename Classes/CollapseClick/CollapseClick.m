...
-(void)reloadCollapseClick {
    ...
    for (int xx = 0; xx < [CollapseClickDelegate numberOfCellsForCollapseClick]; xx++) {
        ...
        cell.ContentView.frame = self.frame;
        ...
        if ([(id)CollapseClickDelegate respondsToSelector:@selector(viewForTittleViewAtIndex:)]) {
            [cell.TitleView setBackgroundColor:[UIColor colorWithPatternImage:[CollapseClickDelegate viewForTittleViewAtIndex:xx]]];
//            [cell.ContentView bringSubviewToFront:cell.TitleArrow];
        }
